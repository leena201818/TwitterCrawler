...
class TwitterCrawler(twython.Twython,metaclass=Singleton):
    ...
    def __init__(self, *args, **kwargs):
        ...
        * apikeys: apikeys
        ...
        self._twuserhelper = TWUserHelper()
    ...
    def fetch_users(self, call = 'screen_name', users = [],savetofile = False,output_folder = './data/users'):
        '''
        call: /users/lookup
        '''
        if not users:
        ...
        filename = os.path.abspath('%s/%s'%(output_folder, now.strftime('%Y%m%d%H%M%S')))
        ...
        while retry_cnt > 0:
            try:
                ...
                if (result):
                    ...
                    # save the result!!
                    logger.debug('save the userinfo of %s to db ' % users[0])
                    self._mongohelper.insertuser(result)
                    retry_cnt = 0

                    if (savetofile):
                        logger.debug('save the result to file %s'%filename)
                        logger.debug(result)
    ...
    def fetch_user_relationships(self, call='/friends/ids', user_id = None,deep = 1,savetofile = False,output_folder = './data/friends_ids'):
        '''
            ...
            try:
                ...
                if (call == '/friends/ids'):
                    if user_id.isdigit():
                        ...
                        result = self.get_friends_ids(screen_name=user_id, cursor=cursor, count=5000)
                ...
                if (result):
                    ...
                    # save the result!!
                    logger.debug('save {0} friends of {1} to db.'.format(len(result['ids']),user_id))
                    self._mongohelper.insertfriends(user_id,result)

                    # dicResult = {'fbid': '111', 'name': 'na', 'homepage': '', 'priority': '1', 'Description': 'ddd',deep:1,'originalfbid':user_id}
                    lstResult = []
                    for id in result['ids']:
                        dicResult = {'fbid': id, 'name': '', 'homepage': '', 'priority': 1, 'Description': user_id,'deep':deep,'originalfbid':user_id}
                        lstResult.append(dicResult)

                    self._twuserhelper.Save_tb_user_friends_batch(lstResult)

                    retry_cnt = 0

                    cursor = result['next_cursor']
                    if (savetofile):
                        logger.debug('save {0} friends of {1} to file {2}'.format(len(result),user_id,filename))
                        with open(filename, 'a+', newline='', encoding='utf-8') as f:
                            f.write('%s\n'%json.dumps(result))
    ...
    def fetch_user_timeline(self, user_id = None, since_id = 1,savetofile = False,output_folder = './data/user_timelines'):
            ...
            try:
                if current_max_id > 0:
                    if(user_id.isdigit()):
                        ...
                        tweets = self.get_user_timeline(screen_name=user_id, tweet_mode='extended', since_id=since_id,max_id=current_max_id - 1, count=200)
                else:
                    if (user_id.isdigit()):
                        tweets = self.get_user_timeline(user_id=user_id, tweet_mode='extended', since_id = since_id, count=200)
                    else:
                        tweets = self.get_user_timeline(screen_name=user_id, tweet_mode='extended', since_id=since_id,count=200)

                prev_max_id = current_max_id # if no new tweets are found, the prev_max_id will be the same as current_max_id

                # save the result!!
                logger.debug('save {0} tweets of {1} to db.'.format(len(tweets),user_id))
                for tweet in tweets:
                    self._mongohelper.inserttweets(user_id,tweet)
                ...
                if (retry_cnt == 0):
                    logger.warn("exceed max retry... return")
                    return 'error',since_id, True # REMOVE the user from the list of track

        logger.info("[%s] total tweets: %d; since_id: [%s]"%(user_id, cnt, since_id))
        return 'ok',current_since_id, False
